...
tryCatch(
    ...
    error = function(e) {
        ...
        assign("params", list(
            sample_or_ref = "aref/default/A.REF"
        ), env = globalenv())
    }
)
...
rmann %>%
    filter(rte_subfamily == "L1HS") %$% req_integrative %>%
    unique()
intact_l1hs <- rmann %>%
    filter(rte_subfamily == "L1HS") %>%
    filter(grepl("^Intact", req_integrative)) %>%
    GRanges()
nonintact_l1hs_fl <- rmann %>%
    filter(rte_subfamily == "L1HS") %>%
    filter(grepl("Full Length", req_integrative)) %>%
    GRanges()
l1hs_fl <- rmann %>%
    filter(rte_subfamily == "L1HS") %>%
    filter(grepl("FL$", rte_length_req)) %>%
    GRanges()
...
sets_of_interest <- gene_sets %>%
    filter(gs_cat == "H") %$% gs_name %>%
    unique()
...

{
    ...
    for (set_name in sets_of_interest) {
        ...
        pt1 <- permTest(
            A = soigr, ntimes = 20, randomize.function = resampleRegions, universe = all.genes,
            evaluate.function = numOverlaps, B = intact_l1hs, verbose = TRUE
        )

        pt2 <- permTest(
            A = intact_l1hs, ntimes = 20, randomize.function = resampleRegions, universe = nonintact_l1hs_fl,
    ...
    for (set_name in hits) {
        ...
        pt1 <- permTest(
            A = soigr, ntimes = 50, randomize.function = resampleRegions, universe = all.genes,
            evaluate.function = numOverlaps, B = intact_l1hs, verbose = TRUE
        )

        pt2 <- permTest(
            A = intact_l1hs, ntimes = 50, randomize.function = resampleRegions, universe = nonintact_l1hs_fl,
            evaluate.function = numOverlaps, B = soigr, verbose = TRUE
        )
...
pt <- permTest(
    A = my.regions, B = repeats, randomize.function = randomizeRegions,
