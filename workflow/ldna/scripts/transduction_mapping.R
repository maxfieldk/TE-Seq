...
dff %>% tibble()
p <- dff %>% filter(Family == "L1") %>% group_by(Chrom, Start, End, Subfamily) %>% summarize(n = n()) %>% arrange(n) %$% n %>% table() %>% as.data.frame() %>%
    mutate(Private = . == 1) %>%
    ggplot(aes(x = ., y = Freq)) +
...
filter(row_number()==1) %>% filter(Filter == "PASS")
    ...
    tryCatch(
        ...
        bres <- tibble(predict(bl, trsd_ss_for_blast)) %>% left_join(insertions_filtered, by = c("QueryID" = "UUID"))
            ...
            bres_hits_grs_prep <- bres_hits %>%
                ...
                rowwise() %>% 
                mutate(start = min(S.start, S.end), end = max(S.start, S.end)) %>% relocate(seqnames, start, end) %>%
            ...
            trsd_adjacent_l1_df <- as.data.frame(trsd_adjacent_l1) %>% tibble() %>% dplyr::select(starts_with("bresgrs."), gene_id) %>%
                dplyr::rename(from_gene_id = gene_id)
            colnames(trsd_adjacent_l1_df) <- gsub("bresgrs.", "", colnames(trsd_adjacent_l1_df))
            trsd_adjacent_l1_df_final <- trsd_adjacent_l1_df %>% group_by(QueryID) %>% mutate(number_productive_hits = n()) %>% filter(Bits == max(Bits)) %>% ungroup()
...
for (index in 1:nrow(TSDcalls)) {
    ...
    from_chr <- TSDcalls[index,]$seqnames %>% str_split("_") %>% unlist() %>% pluck(3)
    from_start <- TSDcalls[index,]$seqnames %>% str_split("_") %>% unlist() %>% pluck(4) %>% as.numeric()
    from_end <- TSDcalls[index,]$seqnames %>% str_split("_") %>% unlist() %>% pluck(5) %>% as.numeric()
    } else {
    ...
    to <- rmann %>% filter(seqnames == insertions_filtered %>% ungroup() %>% filter(UUID == TSDcalls[index,]$QueryID) %$% faName) %>% filter(refstatus == "NonRef")
    to_gene_id <- to[1,] %$% gene_id

    if (grepl("nonref", to[1,]$seqnames)) {
    to_chr <- to[1,]$seqnames %>% str_split("_") %>% unlist() %>% pluck(3)
    to_start <- to[1,]$seqnames %>% str_split("_") %>% unlist() %>% pluck(4) %>% as.numeric()
    to_end <-to[1,]$seqnames %>% str_split("_") %>% unlist() %>% pluck(5) %>% as.numeric()
    }
...
non_ref_nodes <- ttibble_raw %>% left_join(all_intact_l1hs %>% dplyr::select(unique_gene_id, refstatus), by = c("label" = "unique_gene_id")) %>% filter(refstatus == "NonRef") %>% pull(node)
ref_nodes <- ttibble_raw %>% left_join(all_intact_l1hs %>% dplyr::select(unique_gene_id, refstatus), by = c("label" = "unique_gene_id")) %>% filter(refstatus == "Ref") %>% pull(node)
treedf <- ttibble_raw %>% left_join(all_intact_l1hs %>% dplyr::select(unique_gene_id, refstatus), by = c("label" = "unique_gene_id")) %>% filter(!is.na(refstatus))
...
for (index in 1:nrow(treedf_nonref)) {
        ...
        from_chr <- all_intact_l1hs %>% filter(unique_gene_id == from_gene_id) %>% pull(seqnames)
        to_chr <- all_intact_l1hs %>% filter(unique_gene_id == to_gene_id) %>% pull(seqnames) %>% str_extract("chr[0-9XY]+")
        from_start <- all_intact_l1hs %>% filter(unique_gene_id == from_gene_id) %>% pull(start)
        to_start <- all_intact_l1hs %>% filter(unique_gene_id == to_gene_id) %>% pull(start)
        from_end <- all_intact_l1hs %>% filter(unique_gene_id == from_gene_id) %>% pull(end)
        to_end <- all_intact_l1hs %>% filter(unique_gene_id == to_gene_id) %>% pull(end)
...
ttibble_raw <- ttibble_raw %>% left_join(all_intact_l1hs %>% dplyr::select(unique_gene_id, refstatus), by = c("label" = "unique_gene_id")) %>% mutate(label = gsub(".*:", "", label))


ttibblegapped_raw <- ttibblegapped_raw %>% left_join(all_intact_l1hs %>% dplyr::select(unique_gene_id, refstatus), by = c("label" = "unique_gene_id")) %>% mutate(label = gsub(".*:", "", label))
