...
l1hs_promoterregion_hdmr_by_sample_tidy <- read_csv(sprintf("ldna/Rintermediates/%s/%s/highly_demethylated_reads_by_sample.csv", params$mod_code, "L1HS_intactness_req_ALL")) %>%
    dplyr::rename(sample_name = sample)
...
for (sthresh in mrg_l1hs_by_sample_meth$region %>% unique()) {
    tdf <- mrg_l1hs_by_sample_meth %>% filter(region == sthresh)
    ...
    mysaveandstore(str_glue("{outputdir}/L1HS/meth_by_sample_{sthresh}.pdf"), w = 4, h = 4, sf = stats, sfm = model_stats)
...
p <- mf %>% ggplot() +
    ...
    mtclosed +
    anchorbar
mysaveandstore(pl = p, str_glue("{outputdir}/L1HS/hdr_by_sample_all_rsq.pdf"), w = 5, h = 4, sf = mf)
